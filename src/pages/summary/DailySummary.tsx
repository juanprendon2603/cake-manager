...
export function DailySummary() {
  ...
  const formatCurrency = (n: number) =>
    "$" + (n || 0).toLocaleString("es-CO", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  ...
  const { generalExpensesCash, generalExpensesTransfer, generalExpensesTotal } = geTotals;
      ...
      <main className="flex-grow p-6 sm:p-10 max-w-6xl mx-auto w-full">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <KpiCard
            ...
            tone="red"
            sub={`Efec: ${formatCurrency(totalExpensesCash)} · Transf: ${formatCurrency(totalExpensesTransfer)}`}
          />
          <KpiCard
            title="Gastos Generales"
            value={formatCurrency(generalExpensesTotal)}
            tone="red"
            sub={`Efec: ${formatCurrency(generalExpensesCash)} · Transf: ${formatCurrency(generalExpensesTransfer)}`}
          />
          <KpiCard
            title="Neto Global"
            value={(netoGlobal >= 0 ? "+" : "-") + formatCurrency(Math.abs(netoGlobal))}
            tone={netoGlobal >= 0 ? "purple" : "yellow"}
            sub={`Neto (solo diarios): ${(totalNet >= 0 ? "+" : "-") + formatCurrency(Math.abs(totalNet))}`}
          />
        </section>
                ...
                <table className="w-full text-sm">
                  ...
                  <tfoot>
                    <tr className="bg-white font-bold text-purple-900">
                      ...
                      <Td className="text-right">{formatCurrency(generalExpensesTotal)}</Td>
                    </tr>
                  </tfoot>
                </table>
              </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:hidden mt-6">
          {daily.map((row, idx) => {
            const positive = row.net >= 0;
            return (
              <div key={row.fecha} className="p-4 rounded-2xl shadow border border-white/60 bg-white/85 backdrop-blur">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-lg font-bold text-purple-700">{row.fecha}</h3>
                  <Badge tone={positive ? "green" : "red"}>{positive ? "Ganancia" : "Pérdida"}</Badge>
                </div>
